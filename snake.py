...
# checar colisão com corpo da cobrinha
...
# tornar a posição da maçã aleatória
def random_on_grid():
...
while True:
    # para tornar o movimento da cobrinha menor
    pygame.time.Clock().tick(15)
    # para que o caminho da cobrinha seja limpo enquanto ela anda
    screen.fill((0, 0, 0))
    for event in pygame.event.get():
        if event.type == QUIT:
            pygame.quit()
            quit()
            # condição para que a cobrinha seja controlada de acordo com a tecla pressionada
    ...
    # para adicionar novos elementos ao corpo da cobrinha
    ...
    for pos in snake_pos:
        ...
        # para fazer com que a última peça se mova para a próxima peça
    for i in range(len(snake_pos)-1, 0, -1):
        if collision(snake_pos[0], snake_pos[i]):
            restart_game()
        snake_pos[i] = snake_pos[i-1]
        
        if off_limits(snake_pos[i]):
            restart_game()
